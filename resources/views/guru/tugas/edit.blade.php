...
<head>
  ...
  <title>Edit Tugas — Dwira Harapan</title>
...
<div class="main-content">
  ...
  <div class="form-card">
    ...
    <div class="info-box">
      ✏️ Mengedit tugas: <span>{{ $tugas->judul }}</span> —
      Dibuat pada {{ $tugas->created_at->format('d M Y') }}
    </div>
    ...
    <form action="{{ route('guru.tugas.update', $tugas->id) }}" method="POST">
      ...
      <div class="mb-3">
        ...
        <textarea id="deskripsi" name="deskripsi"
          ...
          placeholder="Tuliskan instruksi tugas secara lengkap...">{{ old('deskripsi', $tugas->deskripsi) }}</textarea>
        ...
        <select id="mata_pelajaran" name="mata_pelajaran"
          ...
          @foreach($mataPelajaran as $mp)
            <option value="{{ $mp->nama }}"
              {{ old('mata_pelajaran', $tugas->mata_pelajaran) === $mp->nama ? 'selected' : '' }}>
              {{ $mp->nama }}
            </option>
          @endforeach
        </select>
    ...
    <form action="{{ route('guru.tugas.destroy', $tugas->id) }}" method="POST"
      onsubmit="return confirm('Yakin ingin menghapus tugas ini? Semua kiriman siswa juga akan terhapus.')">
      @csrf @method('DELETE')
      <button type="submit" class="btn-hapus">🗑 Hapus Tugas Ini</button>
    </form>
...
<script>
  ...
  const oldKelas = "{{ old('kelas_target', $tugas->kelas_target) }}";
